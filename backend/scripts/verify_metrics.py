...
# Print detailed URL matching analysis for debugging
def print_debug_url_matching(qrels_items: List[Dict[str, Any]], all_results: List[Any]) -> None:
    ...
    for i, (qrel_item, result) in enumerate(zip(qrels_items, all_results), 1):
        ...
        if relevant_urls:
            ...
            for j, url in enumerate(list(relevant_urls)[:5], 1):  
                print(f"      {j}. {url}")
            if len(relevant_urls) > 5:
                print(f"      ... and {len(relevant_urls) - 5} more")
        else:
            print(f" WARNING: No relevant URLs in QRELS for this query!")
        ...
        if match_count == 0 and relevant_urls and ranked_urls:
            print(f"    NO MATCHES FOUND - Possible reasons:")
    ...
    if queries_with_matches == 0:
        print("\n CRITICAL: NO MATCHES FOUND IN ANY QUERY!")
...
# Main entry point for metrics verification
def main():
    ...
    if passed:
        ...
        if all_zero:
            print("\n ALL METRICS ARE 0.0 - This indicates a fundamental problem:")
            print("\n TROUBLESHOOTING STEPS:")
